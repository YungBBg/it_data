...
export function FoodSetting({ isOpenFoodSetting, foodID }) {
  ...
  const handleRadioChange = (event, title) => {
    setError(false)
    setRadioGroups((prevCount) => {
      const updatedItemList = prevCount.itemList.map((item) => {

        if (Object.keys(item) == title) {
          return {
            ...item,
  ...
  const handleSubmit = (event) => {
    ...
    const allSelect = radioGroups.itemList.map((item) => {
  ...
  const item = storeItems.find(i => i.id === foodID)

  return (
    <Dialog
      open={isOpenFoodSetting}
      onClose={closeFoodSetting}
      scroll={"paper"}
    ...
    >
      ...
      <DialogContent dividers={scroll === 'paper'} >
        ...
        <DialogActions>
            ...
            <Button variant="contained" sx={{ color:'#322805', bgcolor: '#fec81a', ":hover": { bgcolor: "#feda66" } }} onClick={handleSubmit}>Add To Cart</Button>
        </DialogActions>
